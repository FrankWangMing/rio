...
describe('DerivedEventHandlers', () => {
  ...
  beforeEach(() => {
    ...
    const testDerivedEventHandler =
      createTestDerivedHandlers(instance);
  ...
  describe('attaching the connector', () => {
    beforeEach(() => {
      jest.clearAllMocks();
      Object.keys(derivedConnectorsUsage.connectors).forEach(
        (key) => {
    ...
    it('should be able to attach connector', () => {
      const chainedValue =
        derivedConnectorsUsage.connectors.connect(dom);
  ...
  describe('disabling the parent EventHandler instance', () => {
    ...
    it('should only run cleanup', () => {
      Object.keys(derivedConnectorsUsage.connectors).forEach(
        (key) => {
          expect(derivedHandlers[key].cleanup).toHaveBeenCalledTimes(
            1
          );
